#ifndef MCMD_CLUSTER_IDER_CPP
#define MCMD_CLUSTER_IDER_CPP
...
namespace McMd
{
   ...
   * Constructor.
   */
   ClusterIdentifierMC::ClusterIdentifierMC(System& system)
    : links_(),
      clusters_(),
      workStack_(),
      cellList_(),
   ...
   * Destructor.
   */
   ClusterIdentifierMC::~ClusterIdentifierMC()
   {}
   ...
   */
   void
   ClusterIdentifierMC::initialize(int speciesId, int atomTypeId, double cutoff)
   {
   ...
   */
   void ClusterIdentifierMC::processNextMolecule(Cluster& cluster)
   {
   ...
   */
   void ClusterIdentifierMC::identifyClusters()
   {  // Ensure the species is linear SG
      LinearSG* speciesPtr = dynamic_cast<LinearSG*>(&system().simulation().species(speciesId_));
      ...
      for ( ; molIter.notEnd(); ++molIter) {
        // Check to ensure the molecule is the correct subtype, which is considered to always be type 0
        if (speciesPtr->mutator().moleculeStateId(*molIter) == 0) {
   ...
   bool ClusterIdentifierMC::isValid() const
   {
      // Check clusters
