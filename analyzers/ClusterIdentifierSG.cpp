#ifndef MCMD_CLUSTER_IDENTIFIER_SG_CPP
#define MCMD_CLUSTER_IDENTIFIER_SG_CPP
...
namespace McMd
{
   ...
   * Constructor.
   */
   ClusterIdentifierSG::ClusterIdentifierSG(System& system)
    : links_(),
      clusters_(),
      workStack_(),
      cellList_(),
   ...
   * Destructor.
   */
   ClusterIdentifierSG::~ClusterIdentifierSG()
   {}
   ...
   */
   void
   ClusterIdentifierSG::initialize(int speciesId, int atomTypeId, double cutoff, int subType)
   {
   ...
   */
   void ClusterIdentifierSG::processNextMolecule(Cluster& cluster)
   {
   ...
   */
   void ClusterIdentifierSG::identifyClusters()
   {
      ...
      for ( ; molIter.notEnd(); ++molIter) {
         if (speciesPtr->mutator().moleculeStateId(*molIter) ==subTypeId_) {
   ...
   bool ClusterIdentifierSG::isValid() const
   {
      // Check clusters
